-- Test-only setup, run first (alphabetically) by `supabase test db`. Not part
-- of any real migration — pgtap and the tests.* helper functions below only
-- ever exist in the ephemeral local/CI database that `supabase test db`
-- spins up and discards, never in a deployed database.

create extension if not exists pgtap with schema extensions;

-- tests.create_supabase_user() (below) generates ids via uuid_generate_v4(),
-- which isn't the same builtin as the gen_random_uuid() this project's own
-- tables use (that one's core Postgres since v13, no extension needed).
create extension if not exists "uuid-ossp" with schema extensions;

-- ---------------------------------------------------------------------------
-- Vendored from https://github.com/usebasejump/supabase-test-helpers
-- (supabase_test_helpers--0.0.6.sql), per that project's documented "Manual
-- Installation" method: paste the helpers' DDL directly into the first
-- alphabetical test file rather than installing via dbdev/database.dev. That
-- keeps CI from depending on an external package registry being reachable.
-- The statements below run as ordinary autocommitted DDL (this file isn't
-- wrapped in BEGIN/ROLLBACK), so the schema and functions persist for every
-- later test file in this run; the whole database is thrown away afterward.
-- To upgrade: replace this block with a newer supabase_test_helpers--*.sql
-- from that repo and re-run `supabase test db` to confirm the verification
-- block at the bottom still passes.
-- ---------------------------------------------------------------------------
...
-- Verification: confirm the helpers above actually installed, so a broken
-- vendor step fails loudly here instead of producing confusing errors in
