...
- **Inputs/selects:** explicit background, border, and text color in both themes, plus a visible focus ring — `bg-white dark:bg-neutral-900`, `border-neutral-300 dark:border-neutral-700`, `text-neutral-900 dark:text-neutral-100`, `focus:outline-hidden focus:ring-2 focus:ring-blue-500`. Keep input font size at `text-base` (16px) minimum — smaller triggers iOS Safari's auto-zoom on focus.
- **Primary buttons:** invert fully between themes so the button is never close in color to the page background in either theme — `bg-neutral-900 text-white hover:bg-neutral-700 active:bg-neutral-800 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200 dark:active:bg-neutral-300`, plus `focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500 dark:focus-visible:ring-offset-neutral-900` (the ring-offset color must flip too, or the ring gets a light halo in dark mode).
- **Error text:** `text-red-600 dark:text-red-400` (a saturated red reads poorly on dark surfaces) plus `role="alert"` so it's announced to screen readers.
